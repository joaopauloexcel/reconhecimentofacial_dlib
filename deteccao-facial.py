...
def deteccaoTreinamento(totalfotos, pessoaNome, pessoaIdentify):
    cap = cv2.VideoCapture(0) #habilita webcan
    ...
    while True:
        ...
        if k == 27: #se digitar ESC sai
             break
        elif k == ord('s'):#se digitar 's', tira uma foto
            ...
            if facesDetectadas: # se detectou alguma face
                ...
                if fhotos == totalfotos: #se atingiu o total de fotos tiradas
                    ...
                    break
        cv2.imshow('Gravacao', frame) #abre na tela a visualização da webcam
    cap.release()
    cv2.destroyAllWindows()

#chamando método
deteccaoTreinamento(3, 'joao', '123')